...
You can reach one of toy datasets used in our simulation study through [this link](), which points to the folder `data` of this repository.
...
grid_arrange_shared_legend(gg_fig + ggtitle("Estimated"), 
                           ...
                           position = "top")
```
![heatmap_VAF](toy_VAF.png)



## Notes

The variant calling of the single cell sequencing data should be performed by our variant calling pipeline: https://github.com/tianshilu/QBRC-Somatic-Pipeline. One should use the "tumor-only" mode for calling mutations, and set "keep_coverage" (keep coverage data) to 1. Also, please change the Perl script somatic.pl so that `$skip_recal=1` (skip base recalibration), and `$lofreq=1` (use lofreq for scATAC-seq) or =0 (use strelka for scRNA-Seq).
